...
def default_config() -> ClientConfig:
    ...
    pg_agent_config = PolicyGradientAgentConfig(gamma=1, alpha_attacker=0.000008, epsilon=1, render=False,
                                                ...
                                                eval_frequency=125000, attacker=False, defender=True,
                                                video_frequency=101,
                                                save_dir=default_output_dir() + "/results/data",
                                                checkpoint_freq=5000, input_dim_attacker=(4*2 +2) * 3,
...
def run_experiment(configpath: str, random_seed: int, noconfig: bool):
    ...
    logger = util.setup_logger("openai-ppo_vs_maximal_attack-v19", config.output_dir + "/results/logs/" +
                               str(random_seed) + "/",
                               time_str=time_str)
