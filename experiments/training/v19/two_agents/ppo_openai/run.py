...
def default_config() -> ClientConfig:
    ...
    pg_agent_config = PolicyGradientAgentConfig(gamma=1, alpha_attacker=0.00003, epsilon=1, render=False,
                                                ...
                                                one_hot_obs=False, opponent_pool_config=opponent_pool_config,
                                                alternating_optimization=100, opponent_pool=True)
...
def run_experiment(configpath: str, random_seed: int, noconfig: bool):
    ...
    logger = util.setup_logger("openai-ppo_vs_openai-ppo-v19", config.output_dir + "/results/logs/" +
                               str(random_seed) + "/",
                               time_str=time_str)
