...
def default_config() -> ClientConfig:
    ...
    """
    dqn_config = DQNConfig(input_dim=(4 + 2) * 4, attacker_output_dim=(4 + 1) * 4, hidden_dim=64, replay_memory_size=10000,
                           num_hidden_layers=1,
                           ...
                           merged_ad_features=True, state_length=1, normalize_features=False,
    ...
    q_agent_config = QAgentConfig(gamma=0.999, alpha=0.00001, epsilon=1, render=False, eval_sleep=0.9,
                                  ...
                                  video_fps=5, video_dir=default_output_dir() + "/results/videos", num_episodes=200001,
...
def run_experiment(configpath: str, random_seed: int, noconfig: bool):
    ...
    logger = util.setup_logger("dqn_vs_minimal_defense-v9", config.output_dir + "/results/logs/" +
                               str(random_seed) + "/",
                               time_str=time_str)
