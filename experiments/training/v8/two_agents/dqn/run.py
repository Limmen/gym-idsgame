...
def default_config() -> ClientConfig:
    ...
    """
    dqn_config = DQNConfig(input_dim=33, attacker_output_dim=30, defender_output_dim=33,
                           hidden_dim=64, replay_memory_size=10000,
                           num_hidden_layers=1,
...
def run_experiment(configpath: str, random_seed: int, noconfig: bool):
    ...
    logger = util.setup_logger("dqn_vs_random_defense-v8", config.output_dir + "/results/logs/" +
                               str(random_seed) + "/",
                               time_str=time_str)
    ...
    config.q_agent_config.dqn_config.tensorboard_dir = default_output_dir() + "/results/tensorboard/" + str(random_seed) + "/"
