...
def default_config() -> ClientConfig:
    ...
    pg_agent_config = PolicyGradientAgentConfig(gamma=0.999, alpha_attacker=0.00001, epsilon=1, render=False, eval_sleep=0.9,
                                                ...
                                                checkpoint_freq=15000, input_dim_attacker=6 * 2, output_dim_attacker=4,
                                                hidden_dim=64,
...
def run_experiment(configpath: str, random_seed: int, noconfig: bool):
    ...
    logger = util.setup_logger("actor_critic_vs_minimal_defense-v12", config.output_dir + "/results/logs/" +
                               str(random_seed) + "/",
                               time_str=time_str)
